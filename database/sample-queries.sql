...
SELECT 
    ...
    first_seen,
    last_seen,
    EXTRACT(EPOCH FROM (last_seen - first_seen))/3600 as hours_monitored
FROM networks
ORDER BY name;
...
FROM device_status_history d
JOIN networks n ON d.network_id = n.id
WHERE mac_address = 'D8:B6:B7:F1:F8:E4'  -- Replace with actual MAC
ORDER BY timestamp DESC;

-- Count online vs offline devices per network
SELECT 
    n.name,
    SUM(CASE WHEN d.online THEN 1 ELSE 0 END) as currently_online,
...
SELECT 
    mac_address,
    ip_address,
    CASE WHEN online THEN 'ONLINE' ELSE 'OFFLINE' END as status,
    timestamp
FROM device_status_history
WHERE network_id = (SELECT id FROM networks WHERE name = 'MaliGrdi')
ORDER BY timestamp DESC
LIMIT 50;
